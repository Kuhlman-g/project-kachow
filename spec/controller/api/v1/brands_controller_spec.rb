...
RSpec.describe Api::V1::BrandsController, type: :controller do
  ...
  describe "GET#index" do
    it "should return a list of all the brands" do
      ...
      expect(returned_json[0]['name']).to eq("Red Barron")
    end
  end
  ...
  describe "GET#show" do
    it "should display all of the pizzas for a brand" do
      ...
      expect(returned_json['pizzas'][0]['brand_id']).to eq(first_pizza.brand.id)
      expect(returned_json['pizzas'][0]['product_name']).to eq(first_pizza.product_name)
      expect(returned_json['pizzas'][0]['cost']).to eq(first_pizza.cost)
    end
  end
end